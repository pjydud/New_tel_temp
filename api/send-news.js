...
function parseStocks(html) {
  const rows = html.split("<tr"); const items = []; const seen = new Set();
  for (const row of rows) { const codeMatch = row.match(/code=(\d{6})/); const nameMatch = row.match(/class="tltle"[^>]*>([\s\S]*?)<\/a>/); if (!codeMatch || !nameMatch) continue; const code = codeMatch[1]; const name = clean(nameMatch[1]); if (!isCompany(name) || seen.has(code)) continue; seen.add(code); items.push({ name, code }); if (items.length >= 5) break; }
  return items;
}
...
async function personNewsBlock(person) {
  try { const collected = []; const seenLinks = new Set(); const seenTitles = new Set();
    for (const query of person.queries) { const url = `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=ko&gl=KR&ceid=KR:ko`; const xml = await getText(url, "utf-8"); const items = parseNewsItems(xml, query, 3, { daysBack: 7, requiredName: person.name }); for (const item of items) { const titleKey = item.title.replace(/\s+/g, "").slice(0, 50); if (seenLinks.has(item.link) || seenTitles.has(titleKey)) continue; seenLinks.add(item.link); seenTitles.add(titleKey); collected.push(item); } }
    if (!collected.length) return ""; collected.sort((a, b) => Date.parse(b.pubDate || "") - Date.parse(a.pubDate || "")); const item = collected[0]; return `📰 ${escapeHtml(person.label)}\n- ${articleLink(item.title, item.link)}\n  작성일: ${formatKstDateTime(item.pubDate)}`;
  } catch (error) { return ""; }
}

async function fetchHwaseongPolicyNews() { const collected = []; const seenLinks = new Set(); const seenTitles = new Set(); for (const query of HWASEONG_POLICY_QUERIES) { try { const url = `https://news.google.com/rss/search?q=${encodeURIComponent(query)}&hl=ko&gl=KR&ceid=KR:ko`; const xml = await getText(url, "utf-8"); const items = parseNewsItems(xml, query, 10, { daysBack: 7, excludeSoftNews: true, hwaseongPolicyOnly: true }); for (const item of items) { const titleKey = item.title.replace(/\s+/g, "").slice(0, 40); if (seenLinks.has(item.link) || seenTitles.has(titleKey)) continue; seenLinks.add(item.link); seenTitles.add(titleKey); collected.push(item); } } catch (error) {} } collected.sort((a, b) => Date.parse(b.pubDate || "") - Date.parse(a.pubDate || "")); return collected.slice(0, 3); }
